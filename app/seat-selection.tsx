...
export default function SeatSelection() {
  ...
  const toggleSeat = (seatNo: number) => {
    ...
    if (selectedSeats.length >= passengerCount) {
      Alert.alert("Seat Limit", `You can select only ${passengerCount} seat(s)`);
      return;
    }
  ...
  const goToPayment = () => {
    if (selectedSeats.length !== passengerCount) {
      Alert.alert("Select Seats", `Please select exactly ${passengerCount} seat(s)`);
      return;
    }
    const amount = selectedSeats.length * ticketPrice;
    router.push(
      `/payment?busId=${busId}&seats=${selectedSeats.join(",")}&date=${travelDate}&amount=${amount}` as any
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tripCard}>
        <View style={{ flex: 1 }}>
          <Text style={styles.busNameText}>{busName || "HighwayGo Bus"}</Text>
...
const styles = StyleSheet.create({
  ...
  title: { color: "#071A2F", fontSize: 22, fontWeight: "900" },
  ...
  busNameText: { color: "#CFE0FF", fontSize: 14, fontWeight: "800" },
  route: { color: "#FFFFFF", fontSize: 20, fontWeight: "900", marginTop: 6 },
  time: { color: "#FFFFFF", fontSize: 18, fontWeight: "900", textAlign: "right" },
  date: { color: "#CFE0FF", fontSize: 12, marginTop: 8, fontWeight: "700", textAlign: "right" },
  passengerInfo: {
  ...
  passengerInfoText: { color: "#071A2F", fontSize: 13, fontWeight: "900" },
  frontBox: {
  ...
  frontText: { color: "#667085", fontWeight: "900" },
  legend: {
    flexDirection: "row",
    justifyContent: "space-around",
    marginBottom: 18,
  },
  legendText: { color: "#4B5B73", fontSize: 13, fontWeight: "800" },
  busBody: { alignItems: "center", paddingBottom: 25 },
  row: { flexDirection: "row", marginBottom: 13, alignItems: "center" },
  ...
  selectedSeat: { backgroundColor: "#FFD447", borderColor: "#F5A400" },
  bookedSeat: { backgroundColor: "#E8EDF4", borderColor: "#CAD2DF" },
  seatText: { color: "#008C5A", fontSize: 18, fontWeight: "900" },
  selectedSeatText: { color: "#071A2F" },
  bookedSeatText: { color: "#B3BBC8" },
  summaryCard: {
    backgroundColor: "#FFFFFF",
    borderRadius: 22,
    padding: 18,
  ...
  summaryLabel: { color: "#071A2F", fontSize: 14, fontWeight: "900" },
  summarySeats: { color: "#1457D9", fontSize: 16, fontWeight: "900", marginTop: 6 },
  total: { color: "#1457D9", fontSize: 18, fontWeight: "900" },
  totalLabel: { color: "#667085", textAlign: "right", marginTop: 4 },
  confirmButton: {
    backgroundColor: "#1457D9",
    padding: 16,
    borderRadius: 18,
  ...
  confirmText: { color: "#FFFFFF", fontSize: 17, fontWeight: "900" },
});
